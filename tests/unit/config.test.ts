...
describe('loadConfig', () => {
  ...
  it('should read config from config.json when file exists', () => {
    ...
    (readFileSync as jest.Mock).mockReturnValue(
      JSON.stringify({ environmentUrl: 'https://fromfile.crm.dynamics.com', authMode: 'pac' })
    );
    const config = loadConfig();
  ...
  it('should apply TENANT_ID, CLIENT_ID and REDIRECT_URI env var overrides', () => {
    process.env['DATAVERSE_ENV_URL'] = 'https://envtest.crm.dynamics.com';
    process.env['TENANT_ID'] = 'my-tenant';
    process.env['CLIENT_ID'] = 'my-client';
    process.env['REDIRECT_URI'] = 'https://localhost/callback';
    const config = loadConfig();
    expect(config.tenantId).toBe('my-tenant');
    expect(config.clientId).toBe('my-client');
    expect(config.redirectUri).toBe('https://localhost/callback');
  });
