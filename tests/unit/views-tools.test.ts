...
describe("dataverse_list_views", () => {
  ...
  it("returns system views with correct shape", async () => {
    const mock = jest
      .fn()
      .mockResolvedValue({ value: [systemView, systemView2] });
  ...
  it("omits personalViews when includePersonal is false", async () => {
    ...
    const result = await handleViewTool(
      "dataverse_list_views",
      { entityLogicalName: "account", includePersonal: false },
      buildClient(mock),
    );
    const parsed = JSON.parse(result.content[0]!.text) as {
      personalViews: unknown;
      personalViewCount: unknown;
    };
  ...
  it("queries userqueries when includePersonal is true", async () => {
    ...
    const result = await handleViewTool(
      "dataverse_list_views",
      { entityLogicalName: "account", includePersonal: true },
      buildClient(mock),
    );
  ...
  it("passes top correctly", async () => {
    ...
    await handleViewTool(
      "dataverse_list_views",
      { entityLogicalName: "account", top: 50 },
      buildClient(mock),
    );
  ...
  it("rejects top > 100", async () => {
    ...
    await expect(
      handleViewTool(
        "dataverse_list_views",
        { entityLogicalName: "account", top: 101 },
        buildClient(mock),
      ),
    ).rejects.toThrow();
  });
  ...
  it("rejects missing entityLogicalName", async () => {
    const mock = jest.fn();
    await expect(
      handleViewTool("dataverse_list_views", {}, buildClient(mock)),
    ).rejects.toThrow();
  });
...
describe("handleViewTool unknown", () => {
  it("throws on unknown tool name", async () => {
    const mock = jest.fn();
    await expect(
      handleViewTool("dataverse_unknown", {}, buildClient(mock)),
    ).rejects.toThrow("Unknown view tool");
  });
});
