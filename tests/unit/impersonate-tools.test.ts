...
describe('handleImpersonateTool', () => {
  ...
  it('injects MSCRMCallerId during dispatch and removes it afterwards', async () => {
    ...
    await handleImpersonateTool(
      ...
      { callerId: VALID_UUID, toolName: 'dataverse_query', toolArgs: { entitySetName: 'accounts' } },
      mockClient,
      mockDispatch
    );
  ...
  it('calls dispatch with the correct toolName and toolArgs', async () => {
    await handleImpersonateTool(
      'dataverse_impersonate',
      { callerId: VALID_UUID, toolName: 'dataverse_create', toolArgs: { entitySetName: 'contacts', data: { firstname: 'Jane' } } },
      mockClient,
      mockDispatch
    );
  ...
  it('wraps the result with impersonatedAs metadata', async () => {
    ...
    const parsed = JSON.parse(result.content[0]!.text) as { impersonatedAs: string; tool: string; result: unknown };
