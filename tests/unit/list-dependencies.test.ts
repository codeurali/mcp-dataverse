...
describe('dataverse_list_dependencies (FR-TOOL-05)', () => {
  ...
  it('returns structured result with one BusinessRule dependency', async () => {
    const mockResult = {
      ...
      count: 1,
      warning: 'solutionName is not available in P0; solution lookup requires a complex join.',
    };
    ...
    expect(mockClient.listTableDependencies).toHaveBeenCalledWith('account', undefined);
  });
  ...
  it('passes componentType filter array to client when provided', async () => {
    ...
    expect(mockClient.listTableDependencies).toHaveBeenCalledWith('account', ['BusinessRule']);
  });
  ...
  it('returns empty dependencies array when table has no dependencies', async () => {
    ...
    expect(mockClient.listTableDependencies).toHaveBeenCalledWith('new_custom', undefined);
  });
  ...
  it('accepts Plugin and CustomAPI as valid componentType values (C-07)', async () => {
    const mockResult = {
      ...
      dependencies: [],
      count: 0,
      warning: 'solutionName is not available in P0; solution lookup requires a complex join. Plugin and CustomAPI types require additional SDK message queries - not yet implemented. The listed results show Workflow/BusinessRule/Flow dependencies only.',
    };
    mockClient.listTableDependencies!.mockResolvedValue(mockResult);
    ...
    expect(mockClient.listTableDependencies).toHaveBeenCalledWith('account', ['Plugin', 'CustomAPI']);
  });
});
...
describe('dataverse_retrieve_dependencies_for_delete (FR-TOOL-22)', () => {
  ...
  it('calls listDependencies with componentType and objectId', async () => {
    const mockDeps = [{ DependentComponentType: 2, DependentComponentBaseSolutionName: 'MySolution' }];
