...
describe('handleBatchTool', () => {
  ...
  it('returns JSON with results and count on success', async () => {
    ...
    const parsed = JSON.parse(result.content[0]!.text) as { results: typeof batchResults; count: number };
  ...
  it('passes body in batch request when provided', async () => {
    ...
    await handleBatchTool(
      ...
      {
        requests: [
          { method: 'POST', url: '/contacts', body: { firstname: 'Test', lastname: 'User' } },
        ],
      },
      client()
    );
  ...
  it('handles multiple requests in one batch', async () => {
    mockClient.batchExecute!.mockResolvedValue([{ status: 200 }, { status: 204 }]);
  ...
  it('throws ZodError when requests array has more than 1000 items', async () => {
    ...
    await expect(
      handleBatchTool('dataverse_batch_execute', { requests: tooMany }, client())
    ).rejects.toThrow();
  ...
  it('passes useChangeset=true to batchExecute', async () => {
    mockClient.batchExecute!.mockResolvedValue([{ status: 204 }, { status: 204 }]);

    await handleBatchTool(
      'dataverse_batch_execute',
      {
        requests: [
          { method: 'PATCH', url: 'accounts(00000000-0000-0000-0000-000000000001)', body: { name: 'A' } },
          { method: 'PATCH', url: 'accounts(00000000-0000-0000-0000-000000000002)', body: { name: 'B' } },
        ],
        useChangeset: true,
      },
      client()
  ...
  it('101 items are now valid (below the new 1000 limit)', async () => {
    mockClient.batchExecute!.mockResolvedValue(Array.from({ length: 101 }, () => ({ status: 200 })));
    ...
    const result = await handleBatchTool('dataverse_batch_execute', { requests }, client());
    const parsed = JSON.parse(result.content[0]!.text) as { count: number };
  ...
  it('propagates error thrown by client.batchExecute', async () => {
    mockClient.batchExecute!.mockRejectedValue(new Error('Dataverse batch failed'));
