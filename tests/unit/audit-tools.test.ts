...
describe("dataverse_get_audit_log", () => {
  ...
  it("returns mapped entries with parsed changedata", async () => {
    ...
    const result = await handleAuditTool(
    ...
    expect(parsed.entries[0]!.changes).toEqual({
  ...
  it("filters by recordId", async () => {
    ...
    expect(filter).toContain(
      "_objectid_value eq 11111111-1111-1111-1111-111111111111",
    );
  });
  ...
  it("filters by userId", async () => {
    ...
    const filter = mock.mock.calls[0]![1].filter as string;
    expect(filter).toContain(
      "_userid_value eq 22222222-2222-2222-2222-222222222222",
    );
  });
  ...
  it("expands userid", async () => {
    const mock = jest.fn().mockResolvedValue({ value: [] });
    await handleAuditTool("dataverse_get_audit_log", {}, buildClient(mock));
    expect(mock.mock.calls[0]![1].expand).toBe(
      "userid($select=fullname,domainname)",
    );
  });
  ...
  it("returns clear error on 403 (audit not enabled)", async () => {
    const mock = jest
      .fn()
      .mockRejectedValue(new Error("Request failed with status 403"));
    const result = await handleAuditTool(
      "dataverse_get_audit_log",
      {},
      buildClient(mock),
  ...
  it("rethrows non-403 errors", async () => {
    ...
    await expect(
      handleAuditTool("dataverse_get_audit_log", {}, buildClient(mock)),
    ).rejects.toThrow("500");
  });
  ...
  it("maps action code to human name", async () => {
    ...
    const parsed = JSON.parse(result.content[0]!.text) as {
      entries: Array<{ actionName: string; operationName: string }>;
    };
...
describe("handleAuditTool unknown", () => {
  it("throws on unknown tool name", async () => {
    const mock = jest.fn();
    await expect(
      handleAuditTool("dataverse_unknown", {}, buildClient(mock)),
    ).rejects.toThrow("Unknown audit tool");
  });
});
