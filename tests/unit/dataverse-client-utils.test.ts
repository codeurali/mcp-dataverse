import { parseMultipartResponse, esc } from '../../src/dataverse/dataverse-client.utils.js';
...
describe('parseMultipartResponse()', () => {
  ...
  it('parses a single successful 200 response', () => {
    const body = buildMultipart('batch1', [
      mimeWrap('HTTP/1.1 200 OK', JSON.stringify({ value: [{ name: 'Test' }] })),
    ]);
  ...
  it('wraps error responses with { error, status }', () => {
    const errorBody = JSON.stringify({ error: { message: 'Not Found', code: '0x80040217' } });
  ...
  it('handles multiple parts in one response', () => {
    const body = buildMultipart('batchX', [
      ...
      mimeWrap('HTTP/1.1 400 Bad Request', JSON.stringify({ error: { message: 'Bad' } })),
    ]);
