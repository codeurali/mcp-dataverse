...
describe('PacAuthProvider (MSAL)', () => {
  ...
  it('should throw with clear message when no MSAL accounts are found', async () => {
    ...
    const provider = new PacAuthProvider(ENV_URL);
    await expect(provider.getToken()).rejects.toThrow('No authenticated account found');
  });
  ...
  it('setupViaDeviceCode acquires token and caches it', async () => {
    ...
    const callArg = mockAcquireByDeviceCode.mock.calls[0]![0] as { deviceCodeCallback: (r: { message: string }) => void };
  ...
  it('beforeCacheAccess deserializes cache file when it exists', async () => {
    ...
    const ctorArg = MockedPCA.mock.calls[0]![0] as {
      cache: { cachePlugin: { beforeCacheAccess: (ctx: unknown) => Promise<void> } };
    };
    const tokenCacheMock = { deserialize: jest.fn(), serialize: jest.fn() };
    await ctorArg.cache.cachePlugin.beforeCacheAccess({ tokenCache: tokenCacheMock });
  ...
  it('afterCacheAccess writes serialized cache when cacheHasChanged is true', async () => {
    ...
    const ctorArg = MockedPCA.mock.calls[0]![0] as {
      cache: { cachePlugin: { afterCacheAccess: (ctx: unknown) => Promise<void> } };
    };
    const tokenCacheMock = { deserialize: jest.fn(), serialize: jest.fn().mockReturnValue('{"serialized":true}') };
    await ctorArg.cache.cachePlugin.afterCacheAccess({ tokenCache: tokenCacheMock, cacheHasChanged: true });
  ...
  it('afterCacheAccess does not write when cacheHasChanged is false', async () => {
    ...
    const tokenCacheMock = { deserialize: jest.fn(), serialize: jest.fn() };
    await ctorArg.cache.cachePlugin.afterCacheAccess({ tokenCache: tokenCacheMock, cacheHasChanged: false });
