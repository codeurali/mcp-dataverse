import {
  handleQualityTool,
  qualityTools,
} from "../../src/tools/quality.tools.js";
...
describe("quality tool definitions", () => {
  it("exports dataverse_detect_duplicates", () => {
    expect(qualityTools.map((t) => t.name)).toContain(
...
describe("dataverse_detect_duplicates", () => {
  ...
  it("returns hasDuplicates false when no duplicates", async () => {
    const mock = jest.fn().mockResolvedValue({ value: [] });
    const result = await handleQualityTool(
      "dataverse_detect_duplicates",
      {
  ...
  it("rejects missing required fields", async () => {
    const mock = jest.fn();
    await expect(
      handleQualityTool("dataverse_detect_duplicates", {}, buildClient(mock)),
    ).rejects.toThrow();
  });
});
...
describe("handleQualityTool unknown", () => {
  it("throws on unknown tool name", async () => {
    const mock = jest.fn();
    await expect(
      handleQualityTool("dataverse_unknown", {}, buildClient(mock)),
    ).rejects.toThrow("Unknown quality tool");
  });
});
