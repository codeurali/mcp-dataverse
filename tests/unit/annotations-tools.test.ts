import {
  ...
  annotationTools,
} from "../../src/tools/annotations.tools.js";
...
describe("annotation tool definitions", () => {
  it("exports dataverse_get_annotations", () => {
    expect(annotationTools.map((t) => t.name)).toContain(
...
describe("dataverse_get_annotations", () => {
  ...
  it("returns mapped annotations with correct shape", async () => {
    const mock = jest
      .fn()
      .mockResolvedValue({ value: [sampleNote, sampleDoc] });
    const result = await handleAnnotationTool(
      "dataverse_get_annotations",
    ...
    expect(parsed.annotations[0]!.noteText).toBe(
      "Customer requested callback Monday",
    );
  ...
  it("includes documentbody in select when includeContent is true", async () => {
    ...
    await handleAnnotationTool(
      "dataverse_get_annotations",
      { recordId: RECORD_UUID, includeContent: true },
      buildClient(mock),
    );
    const select = mock.mock.calls[0]![1].select as string[];
  ...
  it("adds mimeTypeFilter with single-quote escaping", async () => {
    ...
    await handleAnnotationTool(
      "dataverse_get_annotations",
      { recordId: RECORD_UUID, mimeTypeFilter: "text/plain" },
      buildClient(mock),
    );
    const filter = mock.mock.calls[0]![1].filter as string;
  ...
  it("escapes single quotes in mimeTypeFilter", async () => {
    ...
    await handleAnnotationTool(
      "dataverse_get_annotations",
      { recordId: RECORD_UUID, mimeTypeFilter: "it's/weird" },
      buildClient(mock),
    );
    const filter = mock.mock.calls[0]![1].filter as string;
  ...
  it("rejects missing recordId", async () => {
    const mock = jest.fn();
    await expect(
      handleAnnotationTool("dataverse_get_annotations", {}, buildClient(mock)),
    ).rejects.toThrow();
  });
  ...
  it("extracts owner name from expanded entity", async () => {
    ...
    const parsed = JSON.parse(result.content[0]!.text) as {
      annotations: Array<{ owner: string }>;
    };
...
describe("handleAnnotationTool unknown", () => {
  it("throws on unknown tool name", async () => {
    const mock = jest.fn();
    await expect(
      handleAnnotationTool("dataverse_unknown", {}, buildClient(mock)),
    ).rejects.toThrow("Unknown annotation tool");
  });
});
