...
describe("org tool definitions", () => {
  it("exports dataverse_list_business_units", () => {
    expect(orgTools.map((t) => t.name)).toContain(
...
describe("dataverse_list_business_units", () => {
  ...
  it("returns all BUs including disabled when includeDisabled is true", async () => {
    const mock = jest
      .fn()
      .mockResolvedValue({ value: [rootBU, childBU, disabledBU] });
    await handleOrgTool(
      "dataverse_list_business_units",
  ...
  it("returns mapped units with correct shape", async () => {
    ...
    const result = await handleOrgTool(
      "dataverse_list_business_units",
      {},
      buildClient(mock),
    );
  ...
  it("handles null args (uses defaults)", async () => {
    const mock = jest.fn().mockResolvedValue({ value: [] });
    await expect(
      handleOrgTool("dataverse_list_business_units", null, buildClient(mock)),
...
describe("handleOrgTool unknown", () => {
  it("throws on unknown tool name", async () => {
    const mock = jest.fn();
    await expect(
      handleOrgTool("dataverse_unknown", {}, buildClient(mock)),
    ).rejects.toThrow("Unknown org tool");
  });
});
