...
describe('dataverse_search', () => {
  const sampleResponse = {
    totalrecordcount: 2,
    value: [
      { entityname: 'account', objectid: 'acc-001', score: 0.97, highlights: { name: ['<em>Contoso</em>'] }, attributes: { name: 'Contoso Ltd' } },
      { entityname: 'contact', objectid: 'con-001', score: 0.82, highlights: {}, attributes: { fullname: 'John Contoso' } },
    ],
  };

  it('calls executeAction with relative search URL', async () => {
    const mock = jest.fn().mockResolvedValue(sampleResponse);
    await handleSearchTool('dataverse_search', { query: 'Contoso' }, buildClient(mock));
    expect(mock).toHaveBeenCalledWith('../../search/v1.0/query', expect.objectContaining({ search: 'Contoso' }));
  });

  it('returns mapped results with correct shape', async () => {
    const mock = jest.fn().mockResolvedValue(sampleResponse);
    const result = await handleSearchTool('dataverse_search', { query: 'Contoso' }, buildClient(mock));
    const parsed = JSON.parse(result.content[0]!.text) as { totalRecordCount: number; results: Array<{ entityName: string; objectId: string; score: number }> };
  ...
  it('passes entities array when provided', async () => {
    const mock = jest.fn().mockResolvedValue({ totalrecordcount: 0, value: [] });
    await handleSearchTool('dataverse_search', { query: 'test', entities: ['account', 'contact'] }, buildClient(mock));
  ...
  it('does not include entities key when not provided', async () => {
    const mock = jest.fn().mockResolvedValue({ totalrecordcount: 0, value: [] });
    await handleSearchTool('dataverse_search', { query: 'test' }, buildClient(mock));
    const body = mock.mock.calls[0]![1] as Record<string, unknown>;
  ...
  it('passes searchMode and top', async () => {
    const mock = jest.fn().mockResolvedValue({ totalrecordcount: 0, value: [] });
    await handleSearchTool('dataverse_search', { query: 'test', searchMode: 'all', top: 25 }, buildClient(mock));
    const body = mock.mock.calls[0]![1] as Record<string, unknown>;
  ...
  it('rejects top > 50', async () => {
    const mock = jest.fn();
    await expect(handleSearchTool('dataverse_search', { query: 'test', top: 100 }, buildClient(mock))).rejects.toThrow();
  });

  it('rejects empty query', async () => {
    const mock = jest.fn();
    await expect(handleSearchTool('dataverse_search', { query: '' }, buildClient(mock))).rejects.toThrow();
  });

  it('returns clear error when Relevance Search is not enabled (404)', async () => {
    const mock = jest.fn().mockRejectedValue(new Error('Request failed with status 404'));
    const result = await handleSearchTool('dataverse_search', { query: 'test' }, buildClient(mock));
  ...
  it('rethrows non-404 errors', async () => {
    ...
    await expect(handleSearchTool('dataverse_search', { query: 'test' }, buildClient(mock))).rejects.toThrow('500');
  });
...
describe('handleSearchTool unknown', () => {
  it('throws on unknown tool name', async () => {
    const mock = jest.fn();
    await expect(handleSearchTool('dataverse_unknown', {}, buildClient(mock))).rejects.toThrow('Unknown search tool');
  });
});
