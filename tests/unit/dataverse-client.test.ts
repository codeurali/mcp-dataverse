...
import { DataverseActionsClient } from "../../src/dataverse/dataverse-client.actions.js";
...
describe("DataverseClient", () => {
  ...
  describe("whoAmI", () => {
    it("returns all 5 WhoAmI fields including OrganizationName and EnvironmentUrl", async () => {
      ...
      expect(mockGet).toHaveBeenCalledWith(
        `organizations(${whoAmIData.OrganizationId})?$select=name`,
      );
    });
    ...
    it("returns ECONNABORTED error as timeout message", async () => {
      mockGet.mockRejectedValueOnce(
        createHttpError(0, undefined, "ECONNABORTED"),
      );
  ...
  describe("listTables", () => {
    it("returns table list without filter by default", async () => {
      ...
      expect(mockGet).toHaveBeenCalledWith(
        expect.not.stringContaining("IsCustomEntity eq true"),
      );
    });
    ...
    it("adds filter when includeCustomOnly=true", async () => {
      ...
      expect(mockGet).toHaveBeenCalledWith(
        expect.stringContaining("IsCustomEntity eq true"),
      );
    });
  });
    ...
    it("fetches record by id", async () => {
      ...
      expect(mockGet).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          headers: expect.objectContaining({ Prefer: expect.any(String) }),
        }),
      );
    });
  ...
  describe("createRecord", () => {
    it("extracts id from OData-EntityId header", async () => {
      mockPost.mockResolvedValueOnce({
        data: {},
        headers: {
          "odata-entityid": `${ENV_URL}/api/data/v9.2/accounts(new-id-123)`,
        },
      });
  ...
  describe("upsertRecord", () => {
    it('uses PUT with Prefer header and returns { operation: "created", id } on 201', async () => {
      mockPut.mockResolvedValueOnce({
        ...
        headers: {
          "odata-entityid":
            "https://myorg.crm.dynamics.com/api/data/v9.2/accounts(new-id-123)",
        },
        data: {},
      });
  ...
  describe("associate", () => {
    it("POST to $ref endpoint with @odata.id body", async () => {
      ...
      expect(mockPost).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          "@odata.id": expect.stringContaining("contacts(con-id)"),
        }),
      );
    });
  });
  ...
  describe("executeAction", () => {
    it("POST to named action and returns response data", async () => {
      ...
      const client = new DataverseActionsClient(mockAuthProvider);
      const result = await client.executeAction("SendEmail", {
        target: "email-id",
      });

      expect(result).toEqual({ result: "ok" });
      expect(mockPost).toHaveBeenCalledWith("SendEmail", {
        target: "email-id",
      });
    });
  ...
  describe("executeFunction", () => {
    ...
    it("builds param string from provided parameters", async () => {
      ...
      const client = new DataverseActionsClient(mockAuthProvider);
      await client.executeFunction("MyFunction", {
        EntityName: "account",
        Status: "active",
      });
