...
describe("file tool definitions", () => {
  it("exports dataverse_upload_file_column", () => {
    expect(fileTools.map((t) => t.name)).toContain(
...
function buildClient(httpMock: {
  patch?: jest.Mock;
  get?: jest.Mock;
}): DataverseAdvancedClient {
...
describe("dataverse_upload_file_column", () => {
  ...
  it("calls http.patch with correct URL", async () => {
    const patch = jest
      .fn()
      .mockResolvedValue({ data: null, status: 204, headers: {} });
  ...
  it("returns success result with sizeBytes", async () => {
    const patch = jest
      .fn()
      .mockResolvedValue({ data: null, status: 204, headers: {} });
    const result = await handleFileTool(
      "dataverse_upload_file_column",
      validArgs,
      buildClient({ patch }),
    );
  ...
  it("rejects slash in entitySetName", async () => {
    await expect(
      handleFileTool(
        "dataverse_upload_file_column",
        { ...validArgs, entitySetName: "acc/ounts" },
        buildClient({}),
      ),
    ).rejects.toThrow();
  });
  ...
  it("rejects missing fileContent", async () => {
    const { fileContent: _unused, ...rest } = validArgs; // eslint-disable-line @typescript-eslint/no-unused-vars
    await expect(
      handleFileTool("dataverse_upload_file_column", rest, buildClient({})),
    ).rejects.toThrow();
  });
});
...
describe("dataverse_download_file_column", () => {
  ...
  it("returns base64-encoded content", async () => {
    ...
    const decoded = Buffer.from(parsed.contentBase64, "base64").toString(
      "binary",
    );
...
describe("handleFileTool unknown", () => {
  it("throws on unknown tool name", async () => {
    await expect(
      handleFileTool("dataverse_unknown", {}, buildClient({})),
