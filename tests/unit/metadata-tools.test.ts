...
describe('handleMetadataTool', () => {
  ...
  describe('dataverse_list_tables', () => {
    it('returns JSON list with table data', async () => {
      const tables = [{ LogicalName: 'account', DisplayName: 'Account', EntitySetName: 'accounts' }];
      mockClient.listTables!.mockResolvedValue(tables);

      const result = await handleMetadataTool('dataverse_list_tables', {}, client());
    ...
    it('passes customOnly=true when includeSystemTables=false', async () => {
      ...
      await handleMetadataTool('dataverse_list_tables', { includeSystemTables: false }, client());
    ...
    it('passes customOnly=false when includeSystemTables=true', async () => {
      ...
      await handleMetadataTool('dataverse_list_tables', { includeSystemTables: true }, client());
  ...
  describe('dataverse_get_table_metadata', () => {
    it('returns metadata JSON with attributes', async () => {
      const metadata = {
        LogicalName: 'account',
        Attributes: [{ LogicalName: 'accountid', AttributeType: 'Uniqueidentifier' }],
      };
    ...
    it('passes includeAttributes=false when specified', async () => {
      mockClient.getTableMetadata!.mockResolvedValue({ LogicalName: 'account' });
      ...
      expect(mockClient.getTableMetadata).toHaveBeenCalledWith('account', false);
    });

    it('throws ZodError when logicalName is empty', async () => {
      await expect(
        handleMetadataTool('dataverse_get_table_metadata', { logicalName: '' }, client())
      ).rejects.toThrow();
  ...
  describe('dataverse_get_relationships', () => {
    const makeRels = () => [
      { SchemaName: '1N_rel', RelationshipType: 'OneToManyRelationship', ReferencedEntity: 'account', ReferencingEntity: 'contact' },
      { SchemaName: 'N1_rel', RelationshipType: 'OneToManyRelationship', ReferencedEntity: 'contact', ReferencingEntity: 'account' },
    ...
    it('throws ZodError when logicalName is empty', async () => {
      await expect(
        handleMetadataTool('dataverse_get_relationships', { logicalName: '' }, client())
      ).rejects.toThrow();
    });
  });
  ...
  describe('dataverse_list_global_option_sets', () => {
    it('returns option sets as JSON', async () => {
      ...
      const result = await handleMetadataTool('dataverse_list_global_option_sets', {}, client());
