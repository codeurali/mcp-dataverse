...
describe('teamTools exports', () => {
  it('exports dataverse_list_teams', () => {
    expect(teamTools.find(t => t.name === 'dataverse_list_teams')).toBeDefined();
  });
});
...
describe('handleTeamTool', () => {
  ...
  describe('dataverse_list_teams', () => {
    it('queries the teams entity set', async () => {
      mockClient.query!.mockResolvedValue({ value: [], '@odata.context': 'test' });

      await handleTeamTool('dataverse_list_teams', {}, dvClient());

      expect(mockClient.query).toHaveBeenCalledWith('teams', expect.any(Object));
    });
    ...
    it('omits filter when teamType not provided', async () => {
      ...
      const callArgs = mockClient.query!.mock.calls[0]![1] as Record<string, unknown>;
    ...
    it('returns mapped shape with teamTypeName', async () => {
      ...
      const result = await handleTeamTool('dataverse_list_teams', {}, dvClient());
    ...
    it('returns count 0 and empty teams array when no results', async () => {
      ...
      const parsed = JSON.parse(result.content[0]!.text) as { teams: unknown[]; count: number };
    ...
    it('uses Unknown for unrecognised teamtype values', async () => {
      mockClient.query!.mockResolvedValue({
        value: [{ teamid: 'ccc-333', name: 'Mystery Team', teamtype: 99, isdefault: false }],
        '@odata.context': 'test',
      });
