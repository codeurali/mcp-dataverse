...
describe('CRUD tool handlers', () => {
  ...
  describe('dataverse_create', () => {
    it('returns created record id on success', async () => {
      ...
      const result = await handleCrudTool(
        'dataverse_create',
        { entitySetName: 'accounts', data: { name: 'Test Account', revenue: 50000 } },
        dvClient()
      );
  ...
  describe('dataverse_get', () => {
    ...
    it('returns record data when given a valid UUID', async () => {
      ...
      mockClient.getRecord!.mockResolvedValue({ record: mockRecord, etag: 'W/"12345"' });

      const result = await handleCrudTool(
        'dataverse_get',
        { entitySetName: 'accounts', id: '00000000-0000-0000-0000-000000000001' },
        dvClient()
      );

      const parsed = JSON.parse(result.content[0]!.text) as { record: typeof mockRecord; etag: string | null };
      expect(parsed.record.name).toBe('Contoso Ltd');
    });

    it('includes etag in response when present', async () => {
      const mockRecord = { accountid: '00000000-0000-0000-0000-000000000002', name: 'Fabrikam' };
      mockClient.getRecord!.mockResolvedValue({ record: mockRecord, etag: 'W/"67890"' });

      const result = await handleCrudTool(
        'dataverse_get',
        { entitySetName: 'accounts', id: '00000000-0000-0000-0000-000000000002' },
        dvClient()
      );

      const parsed = JSON.parse(result.content[0]!.text) as { id: string; record: typeof mockRecord; etag: string | null };
    ...
    it('returns etag as null when not present', async () => {
      const mockRecord = { accountid: '00000000-0000-0000-0000-000000000003', name: 'No ETag Corp' };
      mockClient.getRecord!.mockResolvedValue({ record: mockRecord, etag: null });

      const result = await handleCrudTool(
        'dataverse_get',
        { entitySetName: 'accounts', id: '00000000-0000-0000-0000-000000000003' },
        dvClient()
      );

      const parsed = JSON.parse(result.content[0]!.text) as { etag: string | null };
  ...
  describe('dataverse_update', () => {
    ...
    it('throws ZodError when entitySetName is empty', async () => {
      await expect(
        handleCrudTool(
          'dataverse_update',
          { entitySetName: '', id: '00000000-0000-0000-0000-000000000001', data: { name: 'X' } },
          dvClient()
        )
      ).rejects.toThrow();
    });
  ...
  describe('dataverse_delete', () => {
    it('calls deleteRecord and returns success message when confirm=true', async () => {
      ...
      const result = await handleCrudTool(
        'dataverse_delete',
        { entitySetName: 'accounts', id: '00000000-0000-0000-0000-000000000001', confirm: true },
        dvClient()
      );
    ...
    it('blocks deletion and makes no API call when confirm=false', async () => {
      ...
      const result = await handleCrudTool(
        'dataverse_delete',
        { entitySetName: 'accounts', id: '00000000-0000-0000-0000-000000000001', confirm: false },
        dvClient()
      );
  ...
  describe('dataverse_upsert', () => {
    it('returns operation="created" and id on new upsert', async () => {
      mockClient.upsertRecord!.mockResolvedValue({ operation: 'created', id: 'new-record-id-xyz' });
      ...
      const parsed = JSON.parse(result.content[0]!.text) as { operation: string; id: string; message: string };
      ...
      expect(mockClient.upsertRecord).toHaveBeenCalledWith(
        'accounts', 'new_externalid', 'EXT-001', { name: 'Test Account' }, 'upsert', undefined
      );
    });

    it('returns operation="updated" when record already existed', async () => {
      mockClient.upsertRecord!.mockResolvedValue({ operation: 'updated', id: 'existing-id-abc' });
    ...
    it('propagates "Record already exists" error for mode=createOnly', async () => {
      mockClient.upsertRecord!.mockRejectedValue(new Error('Record already exists'));
      ...
      expect(mockClient.upsertRecord).toHaveBeenCalledWith(
        'accounts', 'new_externalid', 'EXT-001', { name: 'Test' }, 'createOnly', undefined
      );
    });
    ...
    it('propagates "Record not found" error for mode=updateOnly', async () => {
      ...
      expect(mockClient.upsertRecord).toHaveBeenCalledWith(
        'contacts', 'new_externalid', 'CONT-001', { firstname: 'Jane' }, 'updateOnly', undefined
      );
    });
  });
...
describe('Query tool handlers', () => {
  ...
  describe('dataverse_query', () => {
    ...
    it('defaults top to 50 and passes options to client.query', async () => {
      mockClient.query!.mockResolvedValue({ value: [], '@odata.context': 'test-ctx' });
      ...
      expect(mockClient.query).toHaveBeenCalledWith(
        'contacts',
        expect.objectContaining({ top: 50, select: ['fullname', 'emailaddress1'] })
      );
