...
async function main(): Promise<void> {
  ...
  const response = await client.executeFetchXml<{ accountid: string; name?: string }>('accounts', FETCH_XML);
  ...
  if (response.value.length === 0) {
    process.stderr.write('  (no account records in this environment — that is OK)\n');
  } else {
    process.stderr.write('  Results:\n');
    response.value.forEach((record, i) => {
      process.stderr.write(`  ${i + 1}. accountid=${record.accountid}  name=${record.name ?? '(none)'}\n`);
    });
  }
