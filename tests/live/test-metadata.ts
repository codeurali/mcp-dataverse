...
async function main(): Promise<void> {
  ...
  if (metadata.LogicalName !== "account") {
    process.stderr.write(
      `✗ Expected LogicalName "account", got "${metadata.LogicalName}"\n`,
    );
    process.exit(1);
  }
  ...
  process.stderr.write(
    `  DisplayName:       ${metadata.DisplayName?.UserLocalizedLabel?.Label ?? "(none)"}\n`,
  );
  process.stderr.write(`  PrimaryIdAttr:     ${metadata.PrimaryIdAttribute}\n`);
  process.stderr.write(
    `  PrimaryNameAttr:   ${metadata.PrimaryNameAttribute}\n`,
  );
