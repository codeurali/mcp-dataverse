...
if (pkg.version !== serverJson.version) {
  fail(`Version mismatch: package.json=${pkg.version}, server.json=${serverJson.version}`);
}
const pkgEntry = serverJson.packages?.[0];
if (pkgEntry?.version && pkgEntry.version !== pkg.version) {
  fail(`Version mismatch: server.json packages[0].version=${pkgEntry.version}, package.json=${pkg.version}`);
}
...
if (pkg.mcpName !== serverJson.name) {
  fail(`mcpName mismatch: package.json.mcpName=${pkg.mcpName}, server.json.name=${serverJson.name}`);
}
ok(`mcpName matches: ${pkg.mcpName}`);

// Check placeholder not still present
if (pkg.mcpName.includes('your-username')) {
  fail('Replace "your-username" with your actual GitHub handle in package.json and server.json');
}
...
if (publishMcp) {
  ...
  try {
    ...
    fail('mcp-publisher not found. Install it first:\n  See PUBLISHING.md step 4');
  }
...
if (publishNpm) console.log(`   npm: https://www.npmjs.com/package/${pkg.name}`);
if (publishMcp) console.log(`   MCP: https://registry.modelcontextprotocol.io/v0.1/servers?search=${serverJson.name}`);
