...
const CreateAnnotationInput = z.object({
    ...
    data => Boolean(data.notetext) || Boolean(data.documentbody),
...
export async function handleAnnotationTool(
    ...
    switch (name) {
        case 'dataverse_get_annotations': {
            ...
            const response = await client.query<Record<string, unknown>>('annotations', {
                select: selectFields,
            ...
            const annotations = rows.map((row) => {
                const result: Record<string, unknown> = {
                    ...
                    owner:
                        (row['ownerid'] as Record<string, unknown> | null)?.['name'] ?? null,
                };
                ...
                content: [
                    {
                        type: 'text',
                        text: JSON.stringify(
                            { recordId: params.recordId, annotations, count: annotations.length },
                            null,
                            2,
                        ),
                    },
