...
export const actionTools = [
  ...
  },
  {
    ...
    description:
      "Executes a global (unbound) Dataverse OData function that is read-only and returns data without side effects — for example RetrieveTotalRecordCount or InitializeFrom. Use dataverse_execute_action for state-changing operations. Use dataverse_execute_bound_action when the function/action requires a specific record context.",
    inputSchema: {
      type: "object" as const,
      properties: {
        functionName: {
          type: "string",
          description:
            'Function name (e.g., "WhoAmI", "RetrieveTotalRecordCount")',
        },
        parameters: {
          type: "object",
  ...
  },
  {
    ...
    description:
      "Checks what solution components would block deletion of a specific component. Provide the Dataverse component type code (1=Entity, 2=Attribute, 26=SavedQuery, 29=Workflow, 92=PluginAssembly) and the component GUID. Use before deleting shared Dataverse customization components.",
    inputSchema: {
      type: "object" as const,
      properties: {
        componentType: {
          type: "number",
          description:
            "Dataverse component type code (1=Entity, 2=Attribute, 29=Workflow, 92=PluginAssembly)",
        },
  ...
  },
  {
    ...
    description:
      "Executes a Dataverse function bound to a specific record (e.g., CalculateRollupField, GetQuantityAvailable). Use for read-only computed operations on a single record. Unlike bound actions, bound functions do not modify data.",
    inputSchema: {
      type: "object" as const,
      properties: {
        entitySetName: {
        ...
        functionName: {
          type: "string",
          description:
            'Name of the bound function (e.g., "CalculateRollupField")',
        },
        parameters: {
          type: "object",
  ...
  },
  {
    ...
    description:
      "Lists workflows, Power Automate flows, Business Rules, and custom actions that reference a given table. Use to detect hidden dependencies before modifying or removing a table. Returns component name, type, state (Active/Draft), trigger event (Create/Update/Delete), and count.",
    inputSchema: {
      type: "object" as const,
      properties: {
        tableName: {
...
const ExecuteActionInput = z.object({
  actionName: z
    ...
    .regex(
      SAFE_API_NAME,
      "actionName must contain only letters, digits, underscores, or dots",
    ),
...
const ExecuteFunctionInput = z.object({
  functionName: z
    ...
    .regex(
      SAFE_API_NAME,
      "functionName must contain only letters, digits, underscores, or dots",
    ),
...
export async function handleActionTool(
  ...
  switch (name) {
    ...
    case "dataverse_execute_bound_action": {
      const { entitySetName, id, actionName, parameters } =
        ExecuteBoundActionInput.parse(args);
      const result = await client.executeBoundAction(
        entitySetName,
        id,
        actionName,
        parameters,
    ...
    case "dataverse_list_dependencies": {
      const { tableName, componentType } =
        ListTableDependenciesInput.parse(args);
      const result = await client.listTableDependencies(
        tableName,
        componentType,
      );
      return {
    ...
    case "dataverse_retrieve_dependencies_for_delete": {
      const { componentType, objectId } =
        RetrieveDependenciesForDeleteInput.parse(args);
    ...
    case "dataverse_execute_bound_function": {
      const { entitySetName, id, functionName, parameters } =
        ExecuteBoundFunctionInput.parse(args);
      const result = await client.executeBoundFunction(
        entitySetName,
        id,
        functionName,
        parameters,
