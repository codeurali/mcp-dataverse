    ...
    inputSchema: {
      ...
      properties: {
        ...
        top: {
          type: "number",
          description:
            "Maximum number of solutions to return (default 50, max 200)",
        },
      },
      required: [],
    },
  ...
  switch (name) {
    case "dataverse_list_solutions": {
      ...
      const solutions = (result.value ?? []).map((s) => ({
        ...
        publisher:
          (s["publisherid"] as Record<string, unknown> | null)?.[
            "friendlyname"
          ] ?? null,
      }));
        ...
        content: [
          {
            type: "text",
            text: JSON.stringify(
              { solutions, count: solutions.length },
              null,
              2,
            ),
          },
    ...
    case "dataverse_solution_components": {
      const {
        ...
        componentType,
        top = 200,
      } = SolutionComponentsInput.parse(args);
      const result = await client.getSolutionComponents(
        solutionName,
        componentType,
        top,
      );
