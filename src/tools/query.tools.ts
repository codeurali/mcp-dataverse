...
export const queryTools = [
    ...
    inputSchema: {
      ...
      properties: {
        entitySetName: {
          type: "string",
          description:
            'The OData entity set name (e.g., "accounts", "contacts", "new_mytables")',
        },
        select: {
          type: "array",
          items: { type: "string" },
        ...
        filter: {
          type: "string",
          description:
            'OData $filter expression (e.g., "statecode eq 0 and new_amount gt 1000")',
        },
        orderby: {
          type: "string",
        ...
        expand: {
          type: "string",
          description:
            'OData $expand for related entities (e.g., "parentaccountid($select=name)")',
        },
        count: {
          type: "boolean",
        ...
        apply: {
          type: "string",
          description:
            'OData $apply for server-side aggregation (e.g., "groupby((statuscode),aggregate($count as count))")',
        },
      },
    ...
    inputSchema: {
      ...
      properties: {
        entitySetName: {
          type: "string",
          description:
            'OData entity set name of the root entity (e.g., "accounts"). If omitted, extracted from the <entity name="..."> element in the FetchXML.',
        },
        fetchXml: {
          type: "string",
  ...
  },
  {
    ...
    description:
      "Retrieves ALL records matching a query by automatically following OData nextLink pages. Use instead of dataverse_query when you need more than 5000 records or all records in a table. Returns totalRetrieved count. Set maxTotal to cap retrieval (default 5000, max 50000) to avoid overwhelming the context.",
    inputSchema: {
      type: "object" as const,
      properties: {
        entitySetName: {
        ...
        maxTotal: {
          type: "number",
          description:
            "Maximum records to retrieve (default: 5000, max: 50000)",
        },
      },
      required: ["entitySetName"],
    },
...
const FetchXmlInput = z.object({
  ...
  entitySetName: z
    ...
    .optional()
    .describe(
      'OData entity set name (e.g., "accounts"). If omitted, extracted from the <entity name="..."> element in the FetchXML.',
    ),
});
...
export async function handleQueryTool(
  ...
  switch (name) {
            ...
            content: [
              {
                ...
                text: JSON.stringify({
                  isError: true,
                  error:
                    "entitySetName is required when not present in FetchXML <entity> element",
                }),
              },
            ],
          };
    ...
    case "dataverse_retrieve_multiple_with_paging": {
      ...
      const result = await client.queryWithPaging(
