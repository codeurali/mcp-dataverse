...
export const metadataTools = [
  {
    ...
    description:
      "Lists all Dataverse tables. By default returns ONLY custom (non-system) tables. Set includeSystemTables=true to include all ~1700+ system tables.",
    inputSchema: {
      ...
      properties: {
        includeSystemTables: {
          type: "boolean",
          description:
            "Include system tables. Default false = custom tables only.",
        },
      },
      required: [],
    },
    ...
    inputSchema: {
      ...
      properties: {
        logicalName: {
          type: "string",
          description:
            'The logical name of the table (e.g., "account", "contact", "new_mytable")',
        },
        includeAttributes: {
          type: "boolean",
  ...
  },
  {
    ...
    description:
      "Returns all option labels and their integer values for a named global option set. Use this to look up the numeric code for a picklist value before filtering records (e.g., statecode or statuscode equivalents), or to populate dropdowns with correct option values.",
    inputSchema: {
      type: "object" as const,
      properties: {
        name: {
  ...
  },
  {
    ...
    description:
      "Returns all alternate key definitions for a Dataverse table. Useful before using dataverse_upsert to know which fields serve as alternate keys, their index status (Active/InProgress/Failed), and whether they are customizable.",
    inputSchema: {
      type: "object" as const,
      properties: {
        tableName: {
  ...
  },
  {
    ...
    description:
      "Returns all option labels and integer values for a table-specific attribute (Picklist, Status, or State field). Use to look up the numeric codes for a column's choices before filtering or updating records.",
    inputSchema: {
      type: "object" as const,
      properties: {
        entityLogicalName: {
        ...
        attributeLogicalName: {
          type: "string",
          description:
            'Logical name of the attribute (e.g., "statuscode", "industrycode")',
        },
      },
...
export async function handleMetadataTool(
  ...
  switch (name) {
    ...
    case "dataverse_get_table_metadata": {
      const { logicalName, includeAttributes } =
        GetTableMetadataInput.parse(args);
      const result = await client.getTableMetadata(
        logicalName,
        includeAttributes,
      );
      return {
    ...
    case "dataverse_get_relationships": {
      const { logicalName, relationshipType } =
        GetRelationshipsInput.parse(args);
      const rawRelationships = await client.getRelationships(logicalName);

      const lname = logicalName.toLowerCase();
      const oneToMany = rawRelationships.filter(
        (r) =>
          r.RelationshipType === "OneToManyRelationship" &&
          r.ReferencedEntity?.toLowerCase() === lname,
      );
      const manyToOne = rawRelationships.filter(
        (r) =>
          r.RelationshipType === "OneToManyRelationship" &&
          r.ReferencingEntity?.toLowerCase() === lname,
      );
      const manyToMany = rawRelationships.filter(
        (r) => r.RelationshipType === "ManyToManyRelationship",
      );
            ...
            text: JSON.stringify(
              {
                ...
                oneToMany:
                  includeAll || relationshipType === "OneToMany"
                    ? oneToMany
                    : undefined,
                manyToOne:
                  includeAll || relationshipType === "ManyToOne"
                    ? manyToOne
                    : undefined,
                manyToMany:
                  includeAll || relationshipType === "ManyToMany"
                    ? manyToMany
                    : undefined,
              },
              null,
        ...
        content: [
          {
            type: "text" as const,
            text: JSON.stringify(
              { tableName, keys, count: keys.length },
              null,
              2,
            ),
          },
