...
export class DataverseAdvancedClient extends DataverseBatchClient {
  ...
  async getChangedRecords<T = Record<string, unknown>>(
    ...
    type DeltaResponse = ODataResponse<Record<string, unknown>> & {
  ...
  async getSolutionComponents(
    ...
    return this.requestWithRetry(async () => {
      const solResp = await this.http.get<{
      ...
      const compResp = await this.http.get<{
        value: Array<Record<string, unknown>>;
      }>(
      ...
      const components = compResp.data.value.map((c) => ({
        ...
        componentTypeName:
          COMPONENT_TYPE_MAP[c["componenttype"] as number] ??
