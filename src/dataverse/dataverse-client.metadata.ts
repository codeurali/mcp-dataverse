...
export class DataverseMetadataClient extends DataverseActionsClient {
  ...
  async listTableDependencies(
    ...
    const response = await this.requestWithRetry(() =>
      this.http
        .get<
          ODataResponse<WorkflowRecord>
        >(`workflows?$filter=primaryentity eq '${esc(tableName)}' and statecode ne 2` + `&$select=name,workflowid,statecode,category,triggeroncreate,triggerondelete,triggeronupdateattributelist`)
        .then((r) => r.data),
    );
    ...
    const hasPluginOrCustomApi = types?.some(
      (t) => t === "Plugin" || t === "CustomAPI",
    );
  ...
  async getAttributeOptionSet(
    ...
    for (const typeName of metadataTypes) {
      try {
        ...
        const optionSet = response["OptionSet"] as
          | Record<string, unknown>
          | undefined;
        const rawOptions =
          (optionSet?.["Options"] as Array<Record<string, unknown>>) ?? [];
        const options = rawOptions.map((o) => {
          const lblObj = o["Label"] as Record<string, unknown> | undefined;
          const locLabel = lblObj?.["UserLocalizedLabel"] as
            | Record<string, unknown>
            | undefined;
          return {
  ...
  async getEntityKeys(tableName: string): Promise<
    Array<{
      schemaName: string;
      logicalName: string;
  ...
  > {
    return this.requestWithRetry(async () => {
      ...
      return response.data.value.map((k) => ({
        ...
        isCustomizable:
          (k["IsCustomizable"] as { Value: boolean } | undefined)?.Value ??
          false,
